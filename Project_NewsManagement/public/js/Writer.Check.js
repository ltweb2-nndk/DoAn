function CheckValueWriter(){
    frm=document.frmAddWriter;
    if(frm.FullName.value.length==0 || frm.Email.value.length==0 || frm.Password.value.length==0 || frm.ConfirmPassword.value.length==0 ||frm.DOB.value.length==0 ||frm.Pseudonym.value.length==0)
    {
