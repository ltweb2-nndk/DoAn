function CheckValueEditor(){
    frm=document.frmAddEditor;
    if(frm.FullName.value.length==0 || frm.Email.value.length==0 || frm.Password.value.length==0 || frm.ConfirmPassword.value.length==0 ||frm.DOB.value.length==0 ||frm.CatID.value=="Chọn Chuyên Mục")
    {
